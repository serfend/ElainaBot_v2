...
async def handle_get_robot_info(request: web.Request):
    ...
    ws_connected = bool(
        bot and bot.ws_client and getattr(bot.ws_client, "_connected", False)
    )
    conn_type = "WebSocket"
    conn_status = (
        "已连接"
        if ws_connected
        else ("等待接收中" if conn_type == "Webhook" else "未连接")
    )
...
async def handle_get_robot_qrcode(request: web.Request):
    ...
    try:
        ...
        async with (
            _aiohttp.ClientSession() as session,
            session.get(
                qr_url, ssl=False, timeout=_aiohttp.ClientTimeout(total=10)
            ) as resp,
        ):
