...
async def handle_get_changelog(request: web.Request):
    try:
        ...
        for c in commits if isinstance(commits, list) else []:
            ...
            try:
                fmt = datetime.fromisoformat(date_str.replace("Z", "+00:00")).strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
            except Exception:
                fmt = "未知时间"
...
async def handle_get_update_progress(request: web.Request):
    try:
        return web.json_response(
            {"success": True, "data": _get_updater().get_progress()}
        )
    except Exception as e:
        return web.json_response({"success": False, "message": str(e)}, status=500)
        ...
        try:
            if body.get("force"):
                await updater.force_update(
                    skip_backup=skip_backup, auto_restart=auto_restart
                )
            elif body.get("version"):
                await updater.update_to_version(
                    body["version"], skip_backup=skip_backup, auto_restart=auto_restart
                )
            else:
                await updater.update_to_latest(
                    skip_backup=skip_backup, auto_restart=auto_restart
                )
        except Exception as e:
...
async def handle_test_mirrors(request: web.Request):
    ...
    all_results = []
    tasks = {
        asyncio.ensure_future(_test_one_mirror(m, 3)): m for m in GITHUB_FILE_MIRRORS
    }
    ...
    while pending:
        ...
        for task in done:
            ...
            try:
                await resp.write(
                    f"data: {_json.dumps(result, ensure_ascii=False)}\n\n".encode()
                )
    ...
    _save_mirror_cache(
        sorted([r for r in all_results if r["success"]], key=lambda r: r["latency"])
    )
...
async def handle_set_custom_mirror(request: web.Request):
    ...
    return web.json_response(
        {"success": True, "message": f"已设置自定义镜像: {mirror or '(自动选择)'}"}
    )
...
async def handle_upload_update(request: web.Request):
    ...
    try:
        ...
        return web.json_response(
            {"success": False, "message": f"保存文件失败: {e}"}, status=500
        )
